...
export const update = async(req, res) => {
    try{
        ...
        const updateEmplyee = await Employee.findByIdAndUpdate(id, req.body, {new: true})
        res.status(201).json(updateEmplyee)
    } catch (error) {
        res.status(500).json({error: "Internal Server Error"}) 
    }
}
...
//         res.status(201).json({error: "Employee terminated!!"})
//     } catch (error) {
//         res.status(500).json({error: "Internal Server Error"}) 
//     }
// }